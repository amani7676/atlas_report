    ...
    <div class="card">
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
            <button 
                ...
                class="btn {{ $location === 2 ? 'btn-primary' : '' }}"
                style="{{ $location === 2 ? 'background: #4361ee; color: white;' : 'background: #6c757d; color: white;' }}"
            >
                <i class="fas fa-paper-plane"></i> پیام‌های ارسالی
            </button>
            <button 
                wire:click="setLocation(1)" 
                class="btn {{ $location === 1 ? 'btn-primary' : '' }}"
                style="{{ $location === 1 ? 'background: #4cc9f0; color: white;' : 'background: #6c757d; color: white;' }}"
            >
            ...
            <button 
                wire:click="setLocation(-1)" 
                class="btn {{ $location === -1 ? 'btn-primary' : '' }}"
                style="{{ $location === -1 ? 'background: #f72585; color: white;' : 'background: #6c757d; color: white;' }}"
            >
        ...
        @if($rawResponse)
            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #dee2e6;">
                ...
                <pre style="direction: ltr; text-align: left; background: white; padding: 10px; border-radius: 4px; overflow-x: auto; font-size: 12px; max-height: 300px;">{{ $rawResponse }}</pre>
            </div>
        @endif
        ...
        @else
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($messages as $message)
                            <tr>
                                <td>
                                    <strong>{{ $message['RecId'] ?? $message['recId'] ?? '-' }}</strong>
                                </td>
                                <td>{{ $message['Sender'] ?? $message['sender'] ?? '-' }}</td>
                                <td>{{ $message['Receiver'] ?? $message['receiver'] ?? '-' }}</td>
                                <td>
                                    <div>
                                        {{ \Illuminate\Support\Str::limit($message['Body'] ?? $message['body'] ?? $message['Text'] ?? $message['text'] ?? '-', 100) }}
                                        @if(mb_strlen($message['Body'] ?? $message['body'] ?? $message['Text'] ?? $message['text'] ?? '') > 100)
                                            <br>
                                            ...
                                            <div class="full-text" style="display: none; margin-top: 5px; padding: 10px; background: #f8f9fa; border-radius: 4px;">
                                                {{ $message['Body'] ?? $message['body'] ?? $message['Text'] ?? $message['text'] ?? '-' }}
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    @if(isset($message['SendDate']) || isset($message['sendDate']))
                                    ...
                                    <td>
                                        @if(isset($message['RecId']) || isset($message['recId']))
                                            <span style="background: #4cc9f0; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px;">
                                                {{ $this->getDeliveryStatusText($message['RecId'] ?? $message['recId']) }}
                                            </span>
                                        @else
                                            <span style="color: #999;">-</span>
                                        @endif
                                ...
                                @endif
                                <td>
                                    @if($location == 2 && (isset($message['RecId']) || isset($message['recId'])))
                                        <button
                                            wire:click="checkDeliveryStatus({{ $message['RecId'] ?? $message['recId'] }})"
            ...
            <div style="margin-top: 20px; text-align: center;">
                ...
                <div style="margin-top: 10px; color: #666; font-size: 14px;">
                    نمایش {{ count($messages) }} پیام (از اندیس {{ $index }})
                </div>
            </div>
        @endif
    </div>
